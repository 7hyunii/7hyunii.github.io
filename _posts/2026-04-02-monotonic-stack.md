...
스택의 LIFO(Last In First Out) 성질을 활용하며, **스택 내 원소들이 항상 단조 증가(오름차순)나 단조 감소(내림차순) 상태를 유지**하도록 관리하는 스택입니다.

* **단조 증가 스택 (Increasing):** 밑에서부터 위로 갈수록 값이 커지는 상태 ($a < b < c$)
* **단조 감소 스택 (Decreasing):** 밑에서부터 위로 갈수록 값이 작아지는 상태 ($a > b > c$)
...
### Step 5. `3` 삽입 시도
- Step 4가 끝난 스택의 `Top`은 `5`로, 삽입을 시도하는 `3`보다 큽니다. 단조 증가를 위해 `5`를 Pop 합니다.
- 그 다음 `Top`도 `4`로, 삽입을 시도하는 `3`보다 큽니다. 단조 증가를 위해 `4`를 Pop 합니다.
- 최종적으로 `3`을 삽입합니다.
```powershell
Monotonic Stack : [1, 3]
Popped : 5, 4
